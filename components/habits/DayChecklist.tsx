    ...
    () =>
      activeCategories
        .map((category) => {
          ...
          const due = all.filter((h) => isHabitDueOn(h, date));
          return {
            category,
            due,
            progress: categoryProgress(category, all, date, isCompleted),
          };
        })
        .filter((section) => section.due.length > 0),
    [activeCategories, byCategory, date, isCompleted],
  );

  /** Anything scheduled on other days, shown collapsed at the bottom. */
  const notDue = useMemo(
    () => activeHabits.filter((h) => !isHabitDueOn(h, date)),
    [activeHabits, date],
  );
      ...
      {notDue.length > 0 ? (
        <details className="group border-t border-line pt-4">
          <summary className="cursor-pointer list-none text-[12.5px] font-medium text-ink-muted transition-colors hover:text-ink">
            {notDue.length} not scheduled today
